...
class Logger_PPO(Logger_Base):
    def __init__(self) -> None:
        super().__init__()
        self.env_count = 0
        self.columns = ["env_step", "reward_mean", "reward_std", "mean_loss", "mean_entropy", "mean_policy_loss", "mean_value_loss"]
        self.dataframes = {}
    ...
    def log_train_results(self, dict_:dict):
        ...
        agents = dict_.keys()
        for k, v in dict_.items():
            episode_len = sum(v["episode_length"])
            episode_len_max = max(episode_len, episode_len_max)
            reward_array = np.array(v["episode_reward"])
            rewards[k] = {'mean':reward_array.mean(),'std':reward_array.std()}

            mean_losses[k] = {'loss': v['loss'], 'entropy': v['entropy'], 'policy': v['policy'],'value': v['value']}
        self.env_count += episode_len_max
        for agent in agents:
            row = {
                "env_step": self.env_count,
                "reward_mean": rewards[agent]['mean'],
                "reward_std": rewards[agent]['std'],
                "mean_loss": mean_losses[agent]['loss'],
                "mean_entropy": mean_losses[agent]['entropy'],
                "mean_policy_loss": mean_losses[agent]['policy'],
                "mean_value_loss": mean_losses[agent]['value']
            }
            print(agent)
            print(row)
...
class Trainer_Base(ABC):
    ...
    def start(self, logging: str | None = 'verbose', max_env_step = 5.0e5):
        if logging == 'verbose':
...
class Trainer_OnPolicy(Trainer_Base):
    ...
    def train(self):
        transes_big = {x:TransitionBatch(self.env.max_len*self.run_count,self.batch_form(x)) for x in self.agents}
        infodict = {}
        for act in self.agents:
            infodict[act] = {'episode_length':[], 'episode_reward':[]}
        for i in range(self.run_count):
            ...
            for x in range(self.env.max_len):
                ...
                if done_main or x == (self.env.max_len - 1):
                    ...
                    break
            for k in transes_big.keys():
                transes_big[k].add(transes[k])
...
class Learner_IPPO(Learner_Base):
    ...
    def eval(self, obs) -> dict[str,np.ndarray]:
        ...
        for name in obs.keys():
            out = self.actor[name](th.tensor(obs[name], dtype=th.float).detach())
            std = out[1]
            mu = out[0]
    ...
    def probabity(self, obs, name):
        ...
        #print(out)
        std = out[1]
        v = th.distributions.Normal(out[0],std)
        return v
    ...
    def learn(self, batch, name = 'agent'):
        ...
        for _ in range(1 + self.offpolicy_iterations):
            val = model(batch['states'])[-1]
            next_val = model(batch['next_states'])[-1]

            pi = self.probabity(batch['states'], name)
            ratio = th.exp(pi.log_prob(batch['actions']) - pi.log_prob(batch['actions']).detach() ) if self.old_pi is None else th.exp((pi.log_prob(batch['actions']) - self.old_pi.log_prob(batch['actions'])))
...
class GymEnvAdapterCont(Enviroment_Base):
    ...
    def get_action_space(self):
        return {"Agent":{"type": "continuous", "shape": self.env.action_space.shape[0]}}

class UnityEnvAdapterVisCont(Enviroment_Base):
    def __init__(self, env_path, seed=1, max_len = 1000, graphics = True):
        ...
        self.env = UnityEnvironment(file_name=env_path, seed=seed, no_graphics= not graphics, no_graphics_monitor=True, additional_args=unity_args)
        self.env.reset()
    ...
    def reset(self):
        ...
        for name in self.behavior_name:
            ...
            arr = decision_steps.obs[0]
            #arr2 = arr.reshape((arr.shape[0],arr.shape[2], arr.shape[3], arr.shape[1])).transpose((0,3,1,2))
            #print(name)
            #rgb_image = np.stack([arr[0,0,:,:], arr[0,1,:,:], arr[0,2,:,:]], axis=-1)
        ...
        for name in actions.keys():
            ...
            for aid in self.agent_ids:
                ...
                if aid in decision_steps:
                    arr = decision_steps[aid].obs[0]
                    #arr = arr.reshape((arr.shape[1], arr.shape[2], arr.shape[0])).transpose((2,0,1))
                    obs.append(arr)
...
class UnityEnvAdapterVecCont(Enviroment_Base):
    def __init__(self, env_path, seed=1, max_len = 1000, graphics = True):
        self.max_len = max_len
        unity_args = [
            "-screen-fullscreen", "0",        
    ...
    def get_observation_space(self):
        d = {}
        for name in self.behavior_name:
            d.update({name:{"shape": self.spec[name].observation_specs[0].shape, "type": "vector"}})
        return d
